...
int send_maxshare(const int server_num, const MaxShare& maxshare,
...
int send_freqshare(const int server_num, const FreqShare& freqshare,
                   const uint64_t n) {
  const int sock = (server_num == 0) ? sockfd0 : sockfd1;
...
int send_linregshare(const int server_num, const LinRegShare& share,
                     const size_t degree) {
  const int sock = (server_num == 0) ? sockfd0 : sockfd1;
...
int send_unvalidated(const size_t n) {
  ...
  sent_bytes += send_DaBit_batch(sockfd1, da1, N);
  sent_bytes += send_AltTriple_batch(sockfd0, trip0, N);
  sent_bytes += send_AltTriple_batch(sockfd1, trip1, N);
  ...
  // std::cout << "unvalidated send:\t" << sec_from(start) << "\n";
  // std::cout << "sent " << N << " (originally " << n << ") unvalidated" << "\n";
  // std::cout << "unvalidated bytes per server: " << sent_bytes / 2 << std::endl;
...
int int_sum_helper(const std::string protocol, const size_t numreqs,
  ...
  for (unsigned int i = 0; i < numreqs; i++) {
    ...
    sent_bytes += send_unvalidated(num_bits);
  }
  delete[] intshare0;
  delete[] intshare1;
...
int xor_helper(const std::string protocol, const size_t numreqs, bool &ans) {
  auto start = clock_start();
  int sent_bytes = 0;
...
int var_helper(const std::string protocol, const size_t numreqs,
  ...
  for (unsigned int i = 0; i < numreqs; i++) {
    ...
    sent_bytes += send_ClientPacket(sockfd0, packet0[i], NMul);
    sent_bytes += send_ClientPacket(sockfd1, packet1[i], NMul);

    sent_bytes += send_unvalidated(num_bits * 3);
...
void var_op_invalid(const std::string protocol, const size_t numreqs) {
  ...
  for (unsigned int i = 0; i < numreqs; i++) {
    ...
    std::cout << i << ": " << real_vals[i] << " = " << shares0[i];
    std::cout << "^" << shares1[i] << ", " << squared << " = ";
...
int linreg_helper(const std::string protocol, const size_t numreqs,
  ...
  for (unsigned int i = 0; i < numreqs; i++) {
    ...
    sent_bytes += send_ClientPacket(sockfd0, packet0[i], NMul);
    sent_bytes += send_ClientPacket(sockfd1, packet1[i], NMul);
    sent_bytes += send_unvalidated(num_bits * total_dabits);
...
void linreg_op(const std::string protocol, const size_t numreqs,
               const size_t degree) {
  if (fmpz_cmp_ui(Int_Modulus, (1ULL << (2 * num_bits)) * numreqs) < 0 ) {
    std::cout << "Modulus bits should be at least ";
    std::cout << (2 * num_bits + LOG2(numreqs)) << "\n";
...
void linreg_op_invalid(const std::string protocol, const size_t numreqs) {
  ...
  std::cout << "Sent " << numvalid << " / " << numreqs;
  std::cout << " valid requests" << std::endl;
...
int heavy_helper(const std::string protocol, const size_t numreqs, const double f,
  ...
  for (unsigned int i = 0; i < numreqs; i++) {
    ...
    // std::cout << std::endl;
    sent_bytes += send_freqshare(0, freqshare0[i], 2 * num_bits);
    sent_bytes += send_freqshare(1, freqshare1[i], 2 * num_bits);
...
void multi_heavy_op(const std::string protocol, const size_t numreqs,
  ...
  if (CLIENT_BATCH) {
    sent_bytes += multi_heavy_helper(protocol, numreqs, count, cfg,
      hash_classify, hash_split, hash_value, count_min);
  } else {
    auto start = clock_start();
    for (unsigned int i = 0; i < numreqs; i++)
      sent_bytes += multi_heavy_helper(protocol, 1, count, cfg,
        hash_classify, hash_split, hash_value, count_min);
    std::cout << "make+send:\t" << sec_from(start) << std::endl;
  }
