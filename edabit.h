...
class CorrelatedStore {
  const unsigned int batch_size;  // How many to make at once
...
public:
  ...
  CorrelatedStore(const int serverfd, const int idx, const char* const server0_ip, const char* const server1_ip, const size_t num_bits, const unsigned int batch_size = 64, const bool lazy = false) 
